<template>
<div class="black margin text" v-for="bask in basketArr" :key="bask.id">
  <div class="text" >
    {{ $route.params.category }}
  <h3>{{bask.title.toUpperCase() }}</h3>
  </div>
  <div>
    <!-- <img :src="model.img"> -->
     <img class="img"  :src="require('@/assets/' + bask.img)">
      </div>
      <div class="text ">
      ...
      <td>
    <button class="bott"
        @click="deleteOfCart(bask)">Удалить</button>
   </td>
 </div>
</template>
